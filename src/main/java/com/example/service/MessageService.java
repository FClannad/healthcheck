...
public class MessageService {
    ...
    public void sendCrawlerMessage(String keyword, int count, String source) {
        try {
            ...
            rabbitTemplate.convertAndSend(
                RabbitMQConfig.HEALTH_EXCHANGE,
                RabbitMQConfig.CRAWLER_ROUTING_KEY,
                message
            );
     ...
     * 处理死信队列消息
     */
    @RabbitListener(queues = "health.dlx.queue")
    public void handleDeadLetterMessage(
        ...
        try {
            ...
            logger.error("处理死信消息失败", e);
            try {
                channel.basicNack(deliveryTag, false, false); // 不重新入队
            } catch (IOException ioException) {
                logger.error("拒绝死信消息失败", ioException);
            }
        }
    }
}
